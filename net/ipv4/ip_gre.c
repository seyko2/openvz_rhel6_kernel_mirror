...
static int __net_init ipgre_init_net(struct net *net)
{
	...
	itn = kzalloc(sizeof(struct ip_tunnel_net), GFP_KERNEL);
	if (itn == NULL)
		return -ENOMEM;
	err = net_assign_generic(net, ipgre_net_id, itn);
	if (err < 0)
		goto free;

	err = ip_tunnel_init_net(net, ipgre_net_id, &ipgre_link_ops, NULL);
	if (err < 0)
		goto free;
	return 0;
...
static int __net_init ipgre_tap_init_net(struct net *net)
{
	...
	if (itn == NULL)
		return -ENOMEM;
	err = net_assign_generic(net, gre_tap_net_id, itn);
	if (err < 0)
		goto free;

	err = ip_tunnel_init_net(net, gre_tap_net_id, &ipgre_tap_ops, NULL);
	if (err < 0)
		goto free;
	return err;
